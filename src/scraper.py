...
async def download_plat_files(df, client):
    """Downloads plat files for each permit in the DataFrame."""
    logging.info("Starting plat file download process.")
    plat_file_paths = []
    ...
    for index, row in df.iterrows():
        ...
        try:
            # Create county-specific directory if it doesn't exist
            download_dir = os.path.join('data', 'plat_files', county)
            os.makedirs(download_dir, exist_ok=True)

            # It's common for these links to be TIF files
            file_extension = ".tif"
            file_path = os.path.join(download_dir, f"{api_no}{file_extension}")

            # Download the file, allowing redirects
            response = await client.get(plat_link, timeout=60, follow_redirects=True)
            response.raise_for_status()

            with open(file_path, 'wb') as f:
                f.write(response.content)

            logging.info(f"Successfully downloaded plat for API {api_no} to {file_path}")
            plat_file_paths.append(file_path)

        except httpx.RequestError as e:
            logging.error(f"Error downloading plat for API {api_no}: {e}")
            plat_file_paths.append('')
        except Exception as e:
            logging.error(f"An unexpected error occurred during plat download for API {api_no}: {e}")
            plat_file_paths.append('')
